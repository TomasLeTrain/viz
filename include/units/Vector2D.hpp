...
public:
  ...
  constexpr Vector2D(Vector2D<Q> other) : x(other.x), y(other.y) {}
   ...
   * @return Vector2D
   */
  static constexpr Vector2D unitVector(Angle t) { return fromPolar(t, (T)1.0); }
...
template <isQuantity T> inline constexpr Vector2D<T> origin({T(0.0), T(0.0)});
...
template <isQuantity Q>
constexpr bool operator==(const Vector2D<Q> &lhs,
                                 const Vector2D<Q> &rhs) {
